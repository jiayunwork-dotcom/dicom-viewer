...
impl DicomParser {
    ...
    fn parse_tags(&mut self) -> Vec<DicomTag> {
...
fn get_tag_value(tags: &[DicomTag], group: u16, element: u16) -> Option<String> {
    tags.iter()
...
pub fn extract_pixel_data(path: &Path, frame_index: u32) -> Result<PixelData, String> {
    ...
    while parser.pos < parser.data.len() {
        ...
        if group == 0x7FE0 && element == 0x0010 {
            let _vr = if parser.is_explicit_vr() {
            ...
            let frame_start = parser.pos + actual_frame * frame_size;
            let frame_end = (frame_start + frame_size).min(parser.pos + pixel_len);

            raw_pixels = decode_pixel_data(
                &parser.data[frame_start..frame_end],
                info.rows,
                info.columns,
                info.bits_allocated,
                info.bits_stored,
...
pub fn generate_thumbnail(pixel_data: &PixelData) -> Vec<u8> {
    ...
    let thumb_w = (pixel_data.width as f64 / scale) as u32;
    let thumb_h = (pixel_data.height as f64 / scale) as u32;
    ...
    let img = image::RgbaImage::from_raw(thumb_w, thumb_h, rgba).unwrap();
